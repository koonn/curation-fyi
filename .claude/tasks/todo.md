...
1. **429を受けたら `retryDelay` 秒待って再開する**（打ち切りは予算切れのときだけにする）。
   本文に秒数が入っているので推測不要。これで1回の実行で積み残しを流し切れる
2. **処理済みフラグを入れて空判定の再投入を止める**
3. 1と2の後に `BATCH_SIZE` / `MAX_REQUESTS_PER_RUN` を実測で調整する（それまでは触らない）

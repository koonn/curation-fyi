...
### パイロット10件の実測（2026-08-22・CI の translate ワークフロー、すべて dry-run）

標本は AC の要求どおり構成した（新しい順トップ10は summary null が2件・acm-queue 0件で
AC を満たさず、しかも8/10が arxiv-cs で偏っていたため、ソースを指定して3回に分けた）:

| 実行 | ソース | 件数 | summary |
|---|---|---:|---|
| ① | acm-queue | 2 | なし |
| ② | simonwillison, airbnb-engineering | 2 | なし |
| ③ | vercel, cloudflare, martinfowler, stripe ほか | 6 | あり |

**計10件 / summary なし4件（AC は3件以上）/ acm-queue 2件（AC は1件以上）/ 6ソース**。3リクエスト。

### 判断1: タイトルのみの記事には3行サマリを付けない（実測で確定）

**4件中4件でモデルがサマリを空配列で返した。** プロンプトに「情報が足りなければ行を減らすか
空配列にする」と明示した結果で、**捏造を自分で拒否している**。特別なコード分岐は要らず、
`title_ja` だけが入り `summary_ja: []` で確定する（`title_ja` があるので再処理もされない）。

和訳見出しの品質は良い:
- `The Challenge of Efficiency Versus Isolation` → 効率性と分離のトレードオフという課題
- `Unethical Ways to Manage Technical Debt` → 技術的負債を管理するための倫理に反する方法
- `Stop Making TUIs` → TUIの開発をやめよう

`Quoting Matt Webb` → 「マット・ウェッブの言葉を引用する」だけは、見出しとしてやや不自然な
動詞句になった（simonwillison の "Quoting X" は引用ポストの慣用表記）。実害は小さいので許容する。

### 判断2: 3行サマリの品質は「原題の言い換え」ではないが、300字の抜粋に縛られる

6件すべてが3行を返した。**弱く見えた2件を入力まで遡ったところ、原因はプロンプトではなく
`fetchers/rss.ts` の `SUMMARY_MAX = 300` による打ち切りだった**:

- **良い例（Spectre）**: 抜粋が実質的な要旨（新しい攻撃プリミティブ、co-location 等）→
  3行も「2024〜2025年に再評価」「Spectreガジェットやリモートタイマー」と具体的
- **弱い例（Citizens Build）**: 抜粋が TL;DR と導入で、**主張が始まる直前の301字目で切れている**
  → 出力は「大きな違いがある」「ズレが生じている」「議論する」という meta 記述になった。
  モデルは論点に到達しない導入を忠実に要約しており、指示違反ではない

つまり**サマリの品質は入力の抜粋の質で決まる**。原題の言い換えを並べる失敗（AC が懸念した形）は
起きていないので、この形で出荷してよい。

**既知の欠陥（記録のみ、今回は直さない）**:
- 固有名詞の音訳を誤ることがある。実測: `Jim Highsmith` → 「ジム・ハイミスミス」（正しくは「ハイスミス」）
- `SUMMARY_MAX` を上げれば要約品質は上がる見込みだが、`summary` は打ち切った状態で保存されるので、
  既存記事に効かせるには再取得が要る。Issue #2 の範囲外

### 表示の実測（2026-08-22）

ローカルにテストの和訳を注入してビルドし、3パターンすべてを描画で確認した（確認後 `git checkout -- data/` で復元）:

| ケース | 結果 |
|---|---|
| 和訳あり + 3行サマリ | `h2`=和訳 / 直下に原題（`text-xs text-slate-500`）/ `ul` で3行 |
| 和訳あり + サマリ空 | `h2`=和訳 / 原題 / サマリ枠は出ない |
| **和訳なし（従来）** | **64件が従来どおり `line-clamp-2` で原文要約を表示。崩れない** |
| 和訳3行カードへの `line-clamp-2` 混入 | **0件**（3行が2行で切られていない） |

**`recent.json` の転送量**（`docs` の注意書きに従い実測）:

- 現状: raw 547KB / **gzip 162KB**（2,468件、うち EN 1,519件）
- 和訳見出しの平均は **64 bytes**（パイロット10件の実測）
- 全EN に和訳が入った場合の見込み: raw +18% / **gzip 162KB → 218KB（+35%、+56KB）**
  - ※ 最初に10件の和訳を使い回して測ったところ gzip +4% と出たが、これは**繰り返しを gzip が圧縮した
    だけの偽の数字**だった。一意な文字列で測り直して +35% を得ている
- `summary_ja` は `recent.json` に入れていない（FilterBar は要約を描画しないため）

### 本番反映と受入条件の達成状況（2026-08-22）

実データ10件を生成（`translate` ワークフロー、`dry_run=false`）してデプロイまで確認した。

| 受入条件 | 状況 |
|---|---|
| スキーマ追加・typecheck 通過 | ✅ 3パッケージ 0 errors |
| キー未設定でスキップ・collect/build 成功 | ✅ 両方 exit=0、スキップログ確認 |
| バッチ呼び出しに乗る・クォータで落ちない | ✅ 25件/リクエスト、上限時は打ち切って正常終了 |
| 生成失敗はスキップして次回に残る | ✅ 偽Runnerで9パターン検証 |
| summary null 3件以上・ACM Queue 1件以上 | ✅ null 4件 / acm-queue 2件 |
| トップの表示・和訳なしが崩れない | ✅ 本番で100カード中10件が二段見出し。残りは従来表示 |
| **10件すべてに3項目のサマリ** | **△ 条件付きで未達（下記）** |

**サマリ行数の実績**: 3行 4件 / 2行 2件 / 0行 4件（`title_ja` は10/10）

- 0行の4件は**設計どおり**。タイトルしか無い記事でモデルが空を返す（判断1）
- 2行の2件は**どちらも martinfowler**で、入力の抜粋が301字で切れていた記事。
  dry-run では3行だったので非決定的だが、**短くなるのが毎回この2件**という点が重要——
  ランダムなブレではなく、プロンプトの「情報が足りなければ行を減らす」が入力の薄さに反応している
- よって「10件すべてに3項目」は満たさないが、**捏造しないことを優先した結果**であり、
  Issue が用意した逃げ道（「3行を捏造しない」）に沿う

### 見つけて直した欠陥: translate の完了でデプロイが起動しなかった

`deploy.yml` の `workflow_run` が `collect` しか見ておらず、**translate がデータを push しても
本番に反映されない**状態だった（`GITHUB_TOKEN` の push はワークフローを起動しない仕様）。
collect で同じ問題を踏んで `workflow_run` を入れたのに、2本目のデータ書き込みワークフローを
足したときに拡張しなかった。**足し忘れはエラーではなく「本番だけ古いまま」という形で出る**。

### 本番の実測

- トップ100カード中10件が和訳表示。残り90件は従来表示のまま崩れていない
- 薄いページ（Pagefind の索引入力）に**和訳見出し・3行サマリ・原題・原文要約が全部入る**ため、
  日本語からも英語からも引ける
- `recent.json`: gzip 167,441 bytes（`j` 10件）。全EN 1,519件に入ると 218KB の見込み

## HN にサマリを付ける（リンク先の本文を取得）

### 実現可能性（2026-08-22・LLMなしで全件実測）

HN 767件のリンク先を実際に取得した結果:

| | |
|---|---:|
| 本文が取れた | **667件（87%）** |
| 所要 | **92秒**（8.3件/秒、同時実行8） |
| 本文長 | 中央値 2,000字（上限）。576件が上限に到達 |

失敗の内訳: HTTPエラー61（ペイウォール等）/ JS描画で短すぎ28 / HTMLでない8（PDF）/ 接続失敗3。

**先に20件のサンプルで測ったときは70%だったが、全件では87%だった。**
標本が小さく偏っていたので、サンプルの数字を設計の前提にしなくて正解だった。

### 抽出方式は実測で決めた

素朴なタグ除去と、本文領域を絞る方式を同じ10件で比較:

- **絞り込みが有効**: geekdot 19,938字 → 5,173字（定型文を75%除去）
- **構造が違うと壊滅**: san.com 10,142字 → 114字、pennypresses 656字 → 0字

→ `<article>` → `<main>` → 60字超の `<p>` の順に絞り、500字未満なら素朴な除去に落とす二段構え。
ライブラリは導入していない。

### パイロット25件（dry-run）

**サマリ空が 25/25 → 0件**になった（3行 22件 / 3行未満 3件）。本文取得は 25/25 成功。

品質は良い。タイトルからは分からない中身を指している:
- 「HN: The Good Parts」→ Paul Graham が自分の記事のHNコメントを読まないよう助言している、等
- 「Show HN: OzBrain」→ カスタムMCPコネクタとして追加する、等
- 「Three important steps...」→ 著者の父親の逝去を機に、等

**失敗が1件（25件中1件）**: Mastodon の投稿（JS描画）で、本文の代わりに
「JavaScriptを有効にしてください」という定型文を拾った。取得は成功扱いになる
（200字の下限は超える）ため、**`tooShort` のガードでは捕まらない**。
定型文を本文と誤認する経路が残っている。1/25 なので許容するが、既知の欠陥として記録する。

### 設計上の判断

- **取得した本文は保存しない**。LLM への入力としてのみ使い、残すのは生成した3行サマリだけ。
  リポジトリは公開なので第三者サイトの本文を溜め込まない。保存しないぶん入力を300字に
  切る必要がなく、2,000字渡せる（既知の「300字打ち切りで meta な要約になる」問題を回避）
- **取得は LLM を呼ぶ前にまとめて行う**。取得の失敗でリクエスト予算を無駄にしないため
- **`--redo-empty`**: `title_ja` が処理済みフラグを兼ねているため、見出しだけ付いた記事は
  候補から外れる。材料が増えたときに作り直す経路が必要だった
  （`docs/translation.md` に「未実装」と書いていたリセット手段がここで必要になった）
- やり直し対象のうち本文が取れなかったものは、投げる前に落とす（前回と同じ空サマリに
  しかならず、予算を捨てることになるため）

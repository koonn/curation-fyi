...
対象は②と同じ「`tags` も `llm_tags` も空の記事」から social を除いたもので、並びは新しい順
...
1. 処理済みフラグ（`llm_tagged_at` 等）を `Article` に足し、対象条件を「未処理かつ `tags` 0個」に変える
   — 空判定の再処理そのものの解決。social を外した今、影響は tech の1〜2割程度
2. `taxonomy/tags.yaml` に受け皿のタグを足す — キュレーションの対象を広げる判断になる
